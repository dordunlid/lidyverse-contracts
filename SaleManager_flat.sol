...
contract LidyERC721 is ERC721, ERC721Enumerable, Pausable, Ownable, ERC721Burnable, ERC2981 {
...
contract Sale is ReentrancyGuard{
    ...
    function whitelistAddresses(address[] memory _address) external {
    ...
    function withdrawFunds() external nonReentrant returns(bool){
        require(getSaleState() == SaleState.ENDED || getSaleState() == SaleState.DIRECT_BUY);
        require(msg.sender == creator);
        (address receiver_, uint256 royaltyAmount) = LidyERC721(nftAddress).royaltyInfo(tokenId,maxBid);
